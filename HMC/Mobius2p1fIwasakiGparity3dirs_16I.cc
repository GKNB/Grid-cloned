...
Source file: ./HMC/Mobius2fIwasakiGparity3dirs_16I.cc
...
//Production binary for the 2f 16I G-parity ensemble
...
void actionCheck(typename GparityMobiusFermionD::ImplParams Params, double light_mass, double pv_mass, double M5, double mob_b, double mob_c, GridCartesian* UGridD, GridRedBlackCartesian* UrbGridD, GridCartesian* FGridD, GridRedBlackCartesian* FrbGridD,
  ...
  XconjFermionActionD::ImplParams xconj_params;
  xconj_params.twists = Params.twists;
  xconj_params.dirichlet = Params.dirichlet;
  xconj_params.boundary_phase = 1.0; //regular x-conjugate
  
  XconjFermionActionD xconj_action_l(Ud,*FGridD,*FrbGridD,*UGridD,*UrbGridD,light_mass,M5,mob_b,mob_c,xconj_params);
  XconjFermionActionD xconj_action_h(Ud,*FGridD,*FrbGridD,*UGridD,*UrbGridD,pv_mass,M5,mob_b,mob_c,xconj_params);

  GparityFermionActionD gp_action_l(Ud,*FGridD,*FrbGridD,*UGridD,*UrbGridD,light_mass,M5,mob_b,mob_c,Params);
  GparityFermionActionD gp_action_h(Ud,*FGridD,*FrbGridD,*UGridD,*UrbGridD,pv_mass,M5,mob_b,mob_c,Params);
...
int main(int argc, char **argv) {
  ...
  ActionLevel<HMCWrapper::Field> Level2(12); //gauge
  ...
  // std::vector<RealD> eofa_light_masses = { light_mass ,  0.03,   0.09,   0.27   };
  // std::vector<RealD> eofa_pv_masses =    { 0.03       , 0.09,   0.27,   1.0      };
  // int n_light_hsb = 4;

  std::vector<RealD> eofa_light_masses = { light_mass ,  0.03,   0.09,  0.18,   0.4   };
  std::vector<RealD> eofa_pv_masses =    { 0.03       , 0.09,   0.18,   0.4,    1.0      };
  int n_light_hsb = 5;
  ...
  bool 
    ...
    eigenrange_s = false, tune_rhmc_s=false,
    action_check = false;
  ...
  for(int i=1;i<argc;i++){
    ...
    if(sarg == "--check_eofa"){
      ...
      eofa_which_hsb = std::stoi(argv[i+1]);
      assert(eofa_which_hsb >= 0 && eofa_which_hsb < n_light_hsb);
    }else if(sarg == "--eigenrange_s"){ //conf 38  measured  0.007234697245907  -  85.3292
      assert(i < argc-1);
      eigenrange_s=true;
      lanc_params_s = argv[i+1];
    }else if(sarg == "--tune_rhmc_s"){
      assert(i < argc-1);
      tune_rhmc_s=true;
  ...
  if(check_eofa || upper_bound_eofa || lower_bound_eofa || eigenrange_s || tune_rhmc_s || action_check) {
