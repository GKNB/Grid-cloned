...
Source file: ./HMC/Mobius2p1fIDSDRXconj_40ID.cc
...
//Production binary for the 40ID G-parity ensemble with X-conj action
...
int main(int argc, char **argv) {
  ...
  typedef XconjugateMobiusEOFAFermionD EOFAfermD;
  typedef XconjugateMobiusFermionD fermionD;
  typedef typename fermionD::Impl_t FermionImplPolicyD;
  typedef typename fermionD::FermionField FermionFieldD;

  typedef XconjugateMobiusEOFAFermionF EOFAfermF;
  typedef XconjugateMobiusFermionF fermionF;
  ...
  //Setup the BCs
  fermionD::ImplParams Params;
  for(int i=0;i<Nd-1;i++) Params.twists[i] = user_params.GparityDirs[i]; //G-parity directions
  Params.twists[Nd-1] = 1; //APBC in time direction  

  typename GparityWilsonTMFermionD::ImplParams GPparams;
  GPparams.twists = Params.twists;
  ...
  ActionLevel<HMCWrapper::Field> Level2(3); //DSDR   4->2 on v7,  2->3 on v8
  ActionLevel<HMCWrapper::Field> Level3(1); //gauge  2->1 on v8

  /////////////////////////////////////////////////////////////
  // Light two-flavor Xconj action
  ...
  //Original
  // std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.016,   0.064,   0.256    };
  // std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.016,   0.064,   0.256,   1.0      };
  // int n_light_hsb = 5;

  //v2
  // std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.009,   0.016,   0.064,   0.256,  0.628    };
  // std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.009,   0.016,   0.064,   0.256,   0.628,  1.0      };
  // int n_light_hsb = 7;

  //v3
  // std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.009,   0.016,   0.064,  0.16,    0.256,  0.628    };
  // std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.009,   0.016,   0.064,   0.16,  0.256,   0.628,  1.0      };
  // int n_light_hsb = 8;

  //v4
  // std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.009,   0.016,   0.064,  0.16,    0.256,  0.442,     0.628    };
  // std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.009,   0.016,   0.064,   0.16,  0.256,    0.442,   0.628,     1.0      };
  // int n_light_hsb = 9;

  //v5
  // std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.009,   0.016,  0.040,  0.064,  0.16,    0.256,  0.442,     0.628    };
  // std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.009,   0.016,   0.040,  0.064,  0.16,  0.256,    0.442,   0.628,     1.0      };
  // int n_light_hsb = 10;

  //v8
  std::vector<RealD> lquark_light_masses = { light_mass ,  0.004,   0.009,   0.016,  0.040,  0.064,  0.112,    0.16,    0.256,  0.442,     0.628    };
  std::vector<RealD> lquark_pv_masses =    { 0.004       , 0.009,   0.016,   0.040,  0.064,  0.112,  0.16,     0.256,    0.442,   0.628,     1.0      };
  int n_light_hsb = 11;
  ...
  //Original
  // std::vector<RealD> squark_light_masses = { strange_mass };
  // std::vector<RealD> squark_pv_masses =    { 1.0 };
  // int n_strange_hsb = 1;

  //v3
  // std::vector<RealD> squark_light_masses = { strange_mass, 0.517 };
  // std::vector<RealD> squark_pv_masses =    { 0.517,        1.0 };
  // int n_strange_hsb = 2;
  ...
  GparityWilsonTMFermionD Numerator_DSDR_D(Ud, *UGridD, *UrbGridD, dsdr_mass, dsdr_epsilon_f, GPparams);
  GparityWilsonTMFermionF Numerator_DSDR_F(Uf, *UGridF, *UrbGridF, dsdr_mass, dsdr_epsilon_f, GPparams);

  GparityWilsonTMFermionD Denominator_DSDR_D(Ud, *UGridD, *UrbGridD, dsdr_mass, dsdr_epsilon_b, GPparams);
  GparityWilsonTMFermionF Denominator_DSDR_F(Uf, *UGridF, *UrbGridF, dsdr_mass, dsdr_epsilon_b, GPparams);
  ...
  if(tune_rhmc_DSDR || eigenrange_DSDR ||check_eofa || upper_bound_eofa || lower_bound_eofa) {
